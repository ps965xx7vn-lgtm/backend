...
- ✅ Регистрацию и аутентификацию (с автоподпиской на уведомления)
- ✅ JWT токены для API
- ✅ Ролевую систему (4 роли для MVP)
- ✅ Email верификацию
- ✅ Сброс пароля
- ✅ Профили пользователей
- ✅ Social authentication (Google, GitHub, etc.)
- ✅ Автоматическая подписка новых пользователей на все типы уведомлений
...
1. **student** - Студент (по умолчанию) - проходит курсы, изучает материалы
2. **mentor** - Ментор - ведет курсы, консультирует студентов
3. **reviewer** - Ревьюер - проверяет работы студентов
4. **manager** - Менеджер - управляет платформой через dashboard

### Profile Models

Расширенные профили для каждой роли:

- **Student** - Информация о студенте (пол, дата рождения, страна, уведомления)
- **Mentor** - Профиль ментора (экспертные области, курсы)
- **Reviewer** - Профиль ревьюера (статистика проверок, экспертизы)
- **Manager** - Профиль менеджера (управление платформой)

**Auto-создание:** Профили создаются автоматически через Django signals при создании пользователя.
**Смена роли:** При смене роли старый профиль деактивируется (is_active=False).
