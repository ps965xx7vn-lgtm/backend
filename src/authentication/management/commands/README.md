# Management Commands

Django management commands для автоматизации задач authentication app.

## 📋 Доступные команды

1. **create_roles** - Создать базовые роли пользователей
2. **create_test_users** - Создать тестовых пользователей для разработки

---

## create_roles

Создает 6 базовых ролей пользователей в системе.

### Использование

```bash
cd src
poetry run python manage.py create_roles
```

### Что создается

| Роль | Название | Описание |
|------|----------|----------|
| `student` | Студент | Обучается на платформе, проходит курсы |
| `mentor` | Ментор | Помогает студентам, проводит консультации |
| `reviewer` | Ревьюер | Проверяет работы студентов |
| `manager` | Менеджер | Управляет платформой и курсами |
| `admin` | Администратор | Полный доступ ко всем функциям |
| `support` | Поддержка | Помогает пользователям решать проблемы |

### Вывод

```
✓ Создана роль: student (Студент)
✓ Создана роль: mentor (Ментор)
✓ Создана роль: reviewer (Ревьюер)
✓ Создана роль: manager (Менеджер)
✓ Создана роль: admin (Администратор)
✓ Создана роль: support (Поддержка)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✓ Команда выполнена успешно!
  Создано ролей: 6
  Уже существовало: 0
  Всего ролей: 6
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

### Особенности

- **Идемпотентна** - можно запускать многократно
- **Атомарна** - использует transaction для целостности
- **Безопасна** - не удаляет существующие роли
- **Информативна** - показывает детальный прогресс

### Когда использовать

- ✅ При первом деплое проекта
- ✅ После миграций базы данных
- ✅ Если роли были случайно удалены
- ✅ В CI/CD pipeline для тестовых окружений

### Пример в коде

```python
from django.core.management import call_command

# Вызвать из Python кода
call_command('create_roles')
```

---

## create_test_users

Создает тестовых пользователей для разработки и тестирования.

### Использование

```bash
# Базовый вариант (6 пользователей - по одному каждой роли)
poetry run python manage.py create_test_users

# С дополнительными студентами
poetry run python manage.py create_test_users --count 10

# Удалить существующих и создать заново
poetry run python manage.py create_test_users --clear
```

### Опции

| Опция | Тип | Описание |
|-------|-----|----------|
| `--count` | int | Количество дополнительных студентов (по умолчанию: 0) |
| `--clear` | flag | Удалить существующих тестовых пользователей перед созданием |

### Созданные пользователи

#### Базовые (всегда создаются)

| Email | Password | Роль | Staff | Superuser |
|-------|----------|------|-------|-----------|
| student@test.com | password123 | student | ❌ | ❌ |
| mentor@test.com | password123 | mentor | ❌ | ❌ |
| reviewer@test.com | password123 | reviewer | ❌ | ❌ |
| manager@test.com | password123 | manager | ✅ | ❌ |
| admin@test.com | password123 | admin | ✅ | ✅ |
| support@test.com | password123 | support | ❌ | ❌ |

#### Дополнительные студенты (с --count)

- `student1@test.com` / `password123`
- `student2@test.com` / `password123`
- `student3@test.com` / `password123`
- ...

### Вывод

```bash
$ poetry run python manage.py create_test_users --count 5

✓ Создан пользователь: student@test.com (роль: student)
✓ Создан пользователь: mentor@test.com (роль: mentor)
✓ Создан пользователь: reviewer@test.com (роль: reviewer)
✓ Создан пользователь: manager@test.com (роль: manager)
✓ Создан пользователь: admin@test.com (роль: admin)
✓ Создан пользователь: support@test.com (роль: support)
✓ Создан студент: student1@test.com
✓ Создан студент: student2@test.com
✓ Создан студент: student3@test.com
✓ Создан студент: student4@test.com
✓ Создан студент: student5@test.com

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✓ Команда выполнена успешно!
  Создано пользователей: 11

Credentials для входа:
  Email: [role]@test.com
  Password: password123
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

### Особенности

- **Проверяет наличие ролей** - требует запуска `create_roles` сначала
- **Безопасна** - не перезаписывает существующих пользователей
- **Атомарна** - использует transaction
- **Гибкая** - можно создать любое количество студентов
- **Email verified** - все пользователи сразу активны

### Когда использовать

- ✅ Локальная разработка
- ✅ Тестирование UI
- ✅ Демонстрации
- ✅ Заполнение тестовой базы
- ❌ **НЕ используйте в production!**

### Пример workflow

```bash
# 1. Создать роли
poetry run python manage.py create_roles

# 2. Создать тестовых пользователей
poetry run python manage.py create_test_users --count 20

# 3. Запустить сервер
poetry run python manage.py runserver

# 4. Войти как admin
# Email: admin@test.com
# Password: password123
```

### Безопасность

⚠️ **ВАЖНО:**
- Команда создает пользователей с **простыми паролями**
- **НЕ запускайте на production**
- Email адреса легко угадываются
- Все пользователи используют один пароль

**Рекомендации:**
- Используйте только для локальной разработки
- Удалите перед деплоем: `--clear` + не запускайте команду
- В production создавайте пользователей через admin или API

---

## Создание своей команды

### Структура

```python
# authentication/management/commands/my_command.py

from django.core.management.base import BaseCommand

class Command(BaseCommand):
    """Краткое описание команды."""
    
    help = 'Подробное описание команды'

    def add_arguments(self, parser):
        """Добавить аргументы командной строки."""
        parser.add_argument(
            '--option',
            type=str,
            help='Описание опции',
        )

    def handle(self, *args, **options):
        """Основная логика команды."""
        self.stdout.write(
            self.style.SUCCESS('Команда выполнена!')
        )
```

### Вызов

```bash
poetry run python manage.py my_command --option value
```

### Best Practices

1. **Docstrings** - всегда документируйте команды
2. **help** - краткое описание для `--help`
3. **Аргументы** - используйте `add_arguments()` для опций
4. **Вывод** - используйте `self.stdout` и `self.style`
5. **Ошибки** - выбрасывайте `CommandError` для ошибок
6. **Транзакции** - используйте `transaction.atomic()` для DB операций
7. **Идемпотентность** - команда должна быть безопасна для повторного запуска

---

## Тестирование команд

```python
from django.core.management import call_command
from django.test import TestCase
from io import StringIO

class TestCreateRolesCommand(TestCase):
    def test_create_roles(self):
        """Тест команды create_roles."""
        out = StringIO()
        call_command('create_roles', stdout=out)
        
        self.assertIn('Создано ролей: 6', out.getvalue())
        
        # Проверить что роли созданы
        from authentication.models import Role
        self.assertEqual(Role.objects.count(), 6)
```

---

## Troubleshooting

### Команда не найдена

```bash
# Проверить файловую структуру
ls authentication/management/commands/

# Должны быть:
# __init__.py
# create_roles.py
# create_test_users.py
```

### Import ошибки

```python
# В commands/__init__.py должно быть:
"""Management commands for authentication app."""

# Не импортируйте команды напрямую в __init__.py!
```

### Роли не создаются

```bash
# Проверить миграции
poetry run python manage.py showmigrations authentication

# Применить если нужно
poetry run python manage.py migrate authentication
```

### Permission denied

```bash
# Убедиться что файлы команд executable (не обязательно для Python)
chmod +x authentication/management/commands/*.py
```

---

## CI/CD Integration

### GitHub Actions

```yaml
name: Setup Test Data

jobs:
  setup:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v2
      - name: Setup roles
        run: |
          cd src
          poetry run python manage.py create_roles
      - name: Setup test users
        run: |
          cd src
          poetry run python manage.py create_test_users --count 10
```

### Docker

```dockerfile
# Dockerfile
RUN poetry run python manage.py create_roles

# docker-compose.yml
services:
  web:
    command: >
      sh -c "poetry run python manage.py create_roles &&
             poetry run python manage.py runserver 0.0.0.0:8000"
```

---

## Статус

✅ **Production Ready**
- Команды протестированы
- Документация полная
- Best practices соблюдены
- Error handling реализован
- Идемпотентность гарантирована
