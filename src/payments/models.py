...
с платежными системами CloudPayments (РФ) и TBC Bank (Грузия).

Модели:
    Payment - Основная модель платежа
        - Статусы: pending, processing, completed, failed, cancelled, refunded
        - Методы оплаты: cloudpayments, tbc_georgia
        - Валюты: USD, GEL, RUB
...
Поля:
    ...
    - payment_method: Способ оплаты (cloudpayments/tbc_georgia)
...
class Payment(models.Model):
    ...
    Поддерживает оплату через CloudPayments (РФ) и TBC Bank (Грузия).
