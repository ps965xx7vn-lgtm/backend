...
В папке `docs/` расположены расширенные документы по dashboard:

### 📊 [DASHBOARD_README.md](docs/DASHBOARD_README.md)

**Описание:** Полная документация по dashboard менеджера с описанием всех компонентов.
...
```python
# views.py - manager_dashboard

@staff_member_required
def manager_dashboard(request):
    """
    Dashboard менеджера со всей статистикой.
    Кешируется на 5 минут.
    """
    # Получаем статистику из кеша
    stats = cache.get('manager_dashboard_stats')

    if not stats:
        # Собираем данные
        stats = {
            'active_users_today': get_active_users_count(today=True),
            'active_users_week': get_active_users_count(days=7),
            'new_registrations': get_new_users_count(days=30),
            'pending_feedback': Feedback.objects.filter(
                is_processed=False
            ).count(),
            'critical_errors': SystemLog.objects.filter(
                level='CRITICAL',
        ...
        cache.set('manager_dashboard_stats', stats, 300)
...
- Redis кеширование тяжелых запросов
- Responsive дизайн

---

### 🔧 [DASHBOARD_USAGE.md](docs/DASHBOARD_USAGE.md)

**Описание:** Инструкция по использованию dashboard для менеджеров.
...
3. Добавить заметку администратора
4. Отметить как обработанное
5. Удалить (если спам)
```
...
# Полезные запросы:
- Критические ошибки за 24ч
- Попытки входа за последний час
...
**Лучшие практики:**
- Ежедневно проверяйте необработанные обращения
- Мониторьте критические ошибки
- Настройте email уведомления
- Регулярно экспортируйте данные для аналитики
