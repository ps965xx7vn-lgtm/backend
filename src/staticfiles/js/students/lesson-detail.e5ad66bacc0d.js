...
/**
 * Mark step as completed via API
 * @param {string} stepId - UUID of the step
 * @returns {Promise}
 */
async function markStepAsCompleted(stepId) {
    // API routes are NOT under language prefix, always at /api/
    const response = await fetch(`/api/students/complete-step/${stepId}/`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': getCookie('csrftoken')
            ...
            nextStepBtn.addEventListener('click', async function() {
                ...
                if (currentStepCard) {
                    ...
                    if (currentIndex >= 0 && currentIndex < allSteps.length - 1) {
                        ...
                        try {
                            await markStepAsCompleted(stepId);
                            ...
                            // Отмечаем чекбокс в левом списке (сайдбар)
