...
class DashboardManager {
    ...
    updateProgress(data) {
        ...
        if (lessonProgressBar && data.lesson_progress) {
            ...
            lessonProgressBar.dataset.percentage = data.lesson_progress.completion_percentage;
        ...
        if (courseProgressBar && data.course_progress) {
            ...
            courseProgressBar.dataset.percentage = data.course_progress.completion_percentage;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize view toggle for history page
    initViewToggle();
});

/**
 * View Toggle для переключения между сеткой и списком
 */
function initViewToggle() {
    const toggleButtons = document.querySelectorAll('.view-toggle-btn');
    const reviewsContainer = document.querySelector('.reviews-container');
    
    if (!toggleButtons.length || !reviewsContainer) return;
    
    // Загружаем сохраненное предпочтение из localStorage
    const savedView = localStorage.getItem('reviewsView') || 'grid';
    reviewsContainer.setAttribute('data-view', savedView);
